require = function e(t, n, r) {
  ...
  test: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "3a3e5BfAjhA1ZLfPlgPXpC8", "test");
    "use strict";
    window.onClientCallJs = function(event, data) {
      alert(event);
      alert(data);
    };
    cc.Class({
      ...
      onBeginRecordClicked: function onBeginRecordClicked() {
        window.webkit.messageHandlers.mmears.postMessage("startRecord", null);
      },
      onEndRecordClicked: function onEndRecordClicked() {
        window.webkit.messageHandlers.mmears.postMessage("stopRecord", null);
      }
    });
    cc._RF.pop();
  }, {} ],
  ...
  utils: [ function(require, module, exports) {
    "use strict";
    cc._RF.push(module, "acb6d6VurZDdrdVYjFGByyV", "utils");
    "use strict";
    var WXSTAT = {
      ...
      init: false,
      configData: false,
      apiList: [ "onMenuShareTimeline", "onMenuShareAppMessage", "onMenuShareQQ", "onMenuShareWeibo", "onMenuShareQZone", "hideOptionMenu", "startRecord", "stopRecord", "onVoiceRecordEnd", "playVoice", "pauseVoice", "stopVoice", "onVoicePlayEnd", "uploadVoice", "downloadVoice", "getNetworkType", "scanQRCode", "showOptionMenu", "hideMenuItems", "showMenuItems", "closeWindow", "scanQRCode", "chooseWXPay" ],
      fns: []
    };
    var Utils = window.MUtils = {
      getOS: function getOS() {
        ...
        /windows\s+nt\s+([\d\.]+)/i.test(ua) ? parseFloat(RegExp.$1) < 6 && (result = "xp") : /macintosh|darwin/i.test(ua) && (result = "mac");
        return result;
      },
      getWxSign: function getWxSign(fn) {
        $.ajax({
          ...
          data.result && fn && fn(data.datum);
        });
      },
      execWxApi: function execWxApi(fn) {
        wx.ready(function() {
          fn && fn();
        });
      },
      checkWxReady: function checkWxReady() {
        if (WXSTAT.configData && WXSTAT.sdkload) {
          wx.config($.extend(WXSTAT.configData, {
            jsApiList: WXSTAT.apiList
          }));
          WXSTAT.ready = true;
          for (var tmp; tmp = WXSTAT.fns.shift(); ) Utils.execWxApi(tmp);
        } else setTimeout(function() {
          Utils.checkWxReady();
        }, 50);
      },
      getWxAuthLink: function getWxAuthLink(eventName, callback) {
      ...
      isMpAuth: function isMpAuth(fn) {
        $.ajax({
          url: "/pay/api/mp_authorized",
          dataType: "json"
        }).done(function(data) {
          if (0 === data.code && data.result && "WX_MP_AUTHORIZED" === data.result.code_name) {
      ...
      isWxAuth: function isWxAuth(fn) {
        $.ajax({
          url: "/api/wechat/pass_token",
          dataType: "json"
        }).done(function(data) {
          data.result && "SUCCESS" === data.result.code_name ? fn(true, data) : fn(false, data);
        }).fail(function() {
          fn(false, {
            fail: true
          });
      ...
      isWxToken: function isWxToken(fn) {
        "_wx_token" in WXSTAT ? fn(WXSTAT["_wx_token"]) : $.ajax({
          url: "/api/wechat/wx_token",
          dataType: "json"
        }).done(function(data) {
          if (data.result && "SUCCESS" === data.result.code_name) {
      ...
      wxSdkInit: function wxSdkInit() {
        if (location.hostname !== WXSTAT.host) return false;
        if (false === WXSTAT.init) {
          WXSTAT.init = true;
          Utils.getWxSign(function(data) {
            WXSTAT.configData = data;
          });
          Utils.loadSdkScript("//res.wx.qq.com/open/js/jweixin-1.2.0.js", function() {
            WXSTAT.sdkload = true;
          });
        }
        Utils.checkWxReady();
      },
      wxReady: function wxReady(fn) {
        if (WXSTAT.ready) Utils.execWxApi(fn); else {
          WXSTAT.fns.push(fn);
          Utils.wxSdkInit();
        }
      },
      loaded: {},
      supportLocal: window.JSON && window.JSON.stringify && window.JSON.parse && window.localStorage && window.localStorage.setItem && window.localStorage.getItem && window.localStorage.removeItem && window.localStorage.key,
      getLocal: function getLocal(key) {
        ...
        if (Utils.supportLocal) {
          ...
          if (!(result && result.expire && result.time)) return result && result.value;
          if (result.expire + result.time > now) return result.value;
      ...
      setLocal: function setLocal(key, value, expire) {
        if (Utils.supportLocal) {
      ...
      loadSdkScript: function loadSdkScript(url, fn) {
        if (Utils.loaded[url]) {
          fn();
          return;
        }
      ...
      loadScript: function loadScript(url, fn) {
      ...
      loadData: function loadData(url, fn) {
        ...
        local ? fn(local) : Utils.getDataByUrl(url, function(data) {
          data && Utils.setLocal(url, data);
          fn(data);
        });
      },
      getDataByUrl: function getDataByUrl(url, fn) {
        $.ajax({
          url: url,
          dataType: "text"
        }).done(function(data) {
      ...
      isPortrait: function isPortrait() {
        if (window.orientation) return 0 === window.orientation || 180 === window.orientation;
        return true;
      },
      isWx: function isWx() {
      ...
      isMobile: function isMobile() {
        var ua = navigator.userAgent;
        if (-1 !== ua.indexOf("Mobile")) return true;
        if (/Android|iPhone|Windows Phone|Nexus|MiuiBrowser/.test(ua)) return true;
        return false;
      },
      getStrkey: function() {
      ...
      requireLogin: function requireLogin(fn) {
        ...
        $.ajax({
          ...
          if (0 === data.code) {
            fn.call(this, data);
            Utils.removeLoading();
          } else 400010010 === data.code && (location.href = data.result);
        }).fail(function() {
      ...
      getStudentInfo: function getStudentInfo(fn) {
        $.ajax({
          url: "/api/student/info",
          dataType: "json"
        }).done(function(data) {
          fn(0 === data.code ? data.result : {});
        }).fail(function() {
          fn({});
        });
      },
      ...
      loadFontsByURL: function loadFontsByURL(url, callback) {
      ...
      sendEvent: function sendEvent(cat, action, label, value) {
        var e = [ "_trackEvent", cat, action ];
        label && e.push(label);
        value && e.push(value);
        window._hmt && window._hmt.push(e);
      },
      sendEventOnce: function sendEventOnce(cat, action, label, value) {
        var val;
      ...
      setFonts: function setFonts(url, name) {
        var st = document.createElement("style");
        st.innerHTML = "@font-face{font-family:" + name + "; src: url(" + url + ");}";
        document.body.appendChild(st);
      },
      loadFonts: function loadFonts(url, name) {
        var dataLocal = window.localStorage.getItem(url);
        dataLocal ? this.setFonts(dataLocal, name) : this.loadFontsByURL(url, function(result) {
          result ? this.setFonts(result, name) : this.setFonts(url, name);
        }.bind(this));
      },
      getParam: function() {
        var params = {};
      ...
      log: function(debug) {
        debug = debug || -1 !== location.search.indexOf("debug");
...
}, {}, [ "account", "game-ctl", "player-data", "bg-controller", "DragonBonesCtrl", "dargon-bone-ctl", "distance-node", "jump", "mask-progress", "move", "stub", "tips-ctl", "game", "player", "coin", "enemy", "pauseLayer", "stub-tag", "stub-controller", "ui-controller", "global", "test", "event-listener", "pool-manager", "resources-manager", "utils" ]);
